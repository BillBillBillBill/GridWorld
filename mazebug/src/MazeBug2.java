...
public class MazeBug extends Bug {
	...
	public int dirprobility[] = {1, 1, 1, 1};
	...
	public int getDirectionByProbility(ArrayList<Location> canMovedDirection) {
		int directionIndex = 0;
		int total = 0;
		for(int i = 0; i < canMovedDirection.size() - 1; i++) {
			int direction = getLocation().getDirectionToward(canMovedDirection.get(i));
			total += dirprobility[direction/90];
		}
		System.out.println("Get total:" + total);
		int randint = new Random().nextInt(total);
		int temp = 0;
		System.out.println("Get random:" + randint);
		for(int i = 0; i < canMovedDirection.size() - 1; i++) {
			int direction = getLocation().getDirectionToward(canMovedDirection.get(i));
			temp += dirprobility[direction/90];
	...
	public boolean canMove() {
		...
		if (isEnd || crossLocation.empty()) {
			...
			if (topArray.size() == 1 || topArray.size() == 0) {
				...
				next = topArray.get(0);
				// dec dirprobility
				System.out.println(getLocation().getDirectionToward(next)/90);

				dirprobility[next.getDirectionToward(getLocation())/90]--;
				System.out.println("Now the weight:" + dirprobility[0] + " " + dirprobility[1]+ " " + dirprobility[2]+ " " + dirprobility[3]);
				return true;
			}
			if (topArray.size() != 0) {
			...
			int randomselectone = getDirectionByProbility(topArray);
			...
			// add probility
			dirprobility[getLocation().getDirectionToward(next)/90]++;
			System.out.println("Now the weight:" + dirprobility[0] + " " + dirprobility[1]+ " " + dirprobility[2]+ " " + dirprobility[3]);
			return true;
		}
	}
	...
	public void move() {
		...
		if (gr.isValid(next)) {
			System.out.println("Go:" + getLocation().getDirectionToward(next));
